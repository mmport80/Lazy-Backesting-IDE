...
function shim(ddd,gts){
        var d=new Graph();
        d.draw(ddd,false,gts);
        }

function Graph(){

        //filter return series or not
        //var filter = null;
        ...
        /*
        orig data is needed to load up
                orig data is global
        update/change graphTimeSeries each time       
                include optional graphTimeSeries input in draw function
        */
        ...
        function update(allReturns){
                ...
                //need to get relevant data
                //add relevant data to tag string
                var importString = '<div id="graph"></div><script type="text/javascript" src="http://storage.googleapis.com/blogjohnorfordcom/Lazy Backtesting/lib/lodash.min.js"></script><script type="text/javascript" src="http://storage.googleapis.com/blogjohnorfordcom/Lazy Backtesting/lib/d3.min.js"></script><script type="text/javascript" src="http://storage.googleapis.com/blogjohnorfordcom/Lazy Backtesting/lib/jstat.min.js"></script><script type="text/javascript" src="http://storage.googleapis.com/blogjohnorfordcom/Lazy Backtesting/lib/graphs.js"></script><link rel="stylesheet" type="text/css" href="http://storage.googleapis.com/blogjohnorfordcom/Lazy Backtesting/graph.css">';
                ...
                var datay = datax
                        ...
                        .map(
                                function(i){
                                        var j = _.pick(i,graphTimeSeries);
                                        j.date = i.date.getFullYear() + "-" + (i.date.getMonth()+1) + "-" + (i.date.getDate());
                                        return j;
                                        }
                                );
                ...
                var s1 = '<script>var a = '+JSON.stringify(datay)+',b='+JSON.stringify(graphTimeSeries)+';</script>'; 
                var s2 = '<script>shim(a,b);</script>';                        
                                
                document.getElementById('codex') != null ? document.getElementById('codex').value = (s1+importString+s2) : null;
        ...
        this.draw = function(data, controlsFlag, gts){
                gts      != null ? graphTimeSeries = gts : null;
        
                datax = data.slice();
                
                //gts != null ? graphTimeSeries = gts : null;
                
                //if controls then filter
                controlsFlag ? filter = true : filter = false;
